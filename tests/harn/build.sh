...
sed 's/function render(){/function render(){window.__S=S;window.__render=render;window.__setFB=function(v){FB=v;};/
     ...
     s/^  function icoValide(k){/  window.__svc={ico:function(i){return svcIco(i);},valide:function(k){return icoValide(k);},choix:function(){return SVC_ICOS;},ajoute:function(n,r){return addCustomService(n,r);},relit:function(d){return applyCatalogDoc(d);},table:function(){return I;}};\n  function icoValide(k){/
  ...
  "$SRC" > "$OUT"
for f in __S __render __err __newMission __peutTravailler __tr __adm __cat __rev __demo __back __svc; do
